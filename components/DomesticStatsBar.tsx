...
function getTrend(rows: DomesticPrice[]) {
...
export default function DomesticStatsBar({ data, isNarrow }: Props) {
  return (
    <div style={{
      display: 'grid',
      ...
      {TRIP_TYPES.map(tripType => {
        const tripRows = data.filter(r => r.trip_type === tripType)

        return (
          <div key={tripType} style={styles.card}>
            <p style={styles.cardTitle}>{TRIP_TYPE_LABELS[tripType]}</p>

            {AIRLINES.map(airline => {
              const rows = tripRows.filter(r => r.airline === airline)
              ...
              const color = DOMESTIC_AIRLINE_COLORS[airline]

              return (
                <div key={airline} style={styles.airlineRow}>
                  ...
                  <div style={{ display: 'flex', gap: 14, marginTop: 6 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <p style={{ ...statValue, fontSize: 13, fontWeight: 500, color: 'var(--text2)' }}>
